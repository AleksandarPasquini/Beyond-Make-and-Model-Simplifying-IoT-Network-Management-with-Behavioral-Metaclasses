...
for smooth in smooths:
    ...
    for test_file in files:
        ...
        test_dataset = tf.data.Dataset.from_generator(
            lambda: data_generator(test_file, batch_size, max_length, return_mac=True),
            output_types=({'input_ids': tf.int32, 'attention_mask': tf.int32}, tf.float32, tf.string),
            output_shapes=(
                {'input_ids': (None, max_length), 'attention_mask': (None, max_length)},
                (None, 7), (None,)
            )
        ...
        for (features, labels, macs) in test_dataset:
            # Get model predictions
            predictions = model.predict(features, verbose = 0)  # shape => (25, 7)

            # Argmax along last axis gives the predicted label per sample
            predicted_labels = np.argmax(predictions, axis=-1)  # shape => (25,)

            # macs is a tf.Tensor of shape (25,), each element is a tf.string
            macs_np = macs.numpy()  # shape => (25,)

            for i in range(batch_size):
                mac_str = macs_np[i].decode('utf-8')           # e.g. "AA:BB:CC:DD:EE:FF"
                pred_label = predicted_labels[i]               # e.g. 0,1,2,...6
                mac_label_counts[mac_str][pred_label] += 1
        print(mac_label_counts)