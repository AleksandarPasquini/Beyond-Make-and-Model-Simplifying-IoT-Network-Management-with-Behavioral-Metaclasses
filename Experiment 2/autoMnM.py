...
def data_generator(file_path, batch_size, max_length, fine, num_labels, pad_token_id=0, limit_per_mac=100000):
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            ...
            y_true_one_hot = tf.one_hot(fine[mac], depth=num_labels)
...
batch_size = 25
test_file = "/scratch/apasquini/Data/Deakin/Deakin_1tokens.json"
...
for (phase, Uset, Dset) in phases:
    ...
    train_dataset = tf.data.Dataset.from_generator(
        lambda: data_generator(train_file, batch_size, max_length, Uset, num_labels),
    ...
    with strategy.scope():
        ...
        y = tf.keras.layers.RepeatVector(max_length)(x)
        y = TransformerDecoder(intermediate_dim=64, num_heads=2, dropout=0.2, normalize_first=True)(
            y,
        )
        y = TransformerDecoder(intermediate_dim=96, num_heads=2, dropout=0.2, normalize_first=True)(
            y
        )
        y = TransformerDecoder(intermediate_dim=128, num_heads=2, dropout=0.2, normalize_first=True)(
            y
        )

        y = LayerNormalization()(y)
        pooled_max = GlobalMaxPooling1D()(y)
        pooled_avg = GlobalAveragePooling1D()(y)
        y = Concatenate()([pooled_max, pooled_avg])
        decoded = Dense(num_labels, activation="softmax") (y)


        # 4. Build a functional model that accepts both inputs
        autoencoder_model = tf.keras.Model(inputs=[input_ids, attention_mask], outputs=decoded)
        autoencoder_model.compile(optimizer='AdamW', loss= tf.keras.losses.CategoricalCrossentropy(), metrics=['accuracy'])
    ...
    test_dataset = tf.data.Dataset.from_generator(
        lambda: data_generator(test_file, batch_size, max_length, Dset, num_labels),
        output_types=({'input_ids': tf.int32, 'attention_mask': tf.int32}, tf.float32),
        output_shapes=(
            {'input_ids': (None, max_length), 'attention_mask': (None, max_length)},
            (None, num_labels)
    ...
    test_loss, test_accuracy = autoencoder_model.evaluate(test_dataset)
