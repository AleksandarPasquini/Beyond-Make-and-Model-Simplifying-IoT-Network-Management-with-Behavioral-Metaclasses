...
batch_size = 8
files = ["/scratch/apasquini/Data/LSIF/LSIF_2d.npz",
        "/scratch/apasquini/Data/Aalto/Aalto_2d.npz",
        "/scratch/apasquini/Data/Deakin/Deakin_2d.npz",
        "/scratch/apasquini/Data/CIC/CIC_2d.npz",
        "/scratch/apasquini/Data/UNSW/UNSW_2d.npz"]
...
for smooth in smooths:
        ...
        if mac_counts[m] < 100000:
            if mac_5:
                ...
                if len(mac_buffers[m]) == 5:
                    truncated_inps = [b_inp.reshape(b_inp.shape[0], -1)[:200] for (b_inp, _) in mac_buffers[m]]
                    combined_inp = tf.concat(truncated_inps, axis=0)  # shape [1000]
                    desired_shape = largest_factor_pair(combined_inp)
                    inp = np.reshape(1,*desired_shape)
                ...
                if len(lab_buffers[label]) == 5:
                    truncated_inps = [b_inp.reshape(b_inp.shape[0], -1)[:200] for (b_inp, _) in lab_buffers[label]]
                    combined_inp = tf.concat(truncated_inps, axis=0)  # shape [1000]
                    desired_shape = largest_factor_pair(combined_inp)
                    inp = np.reshape(1,*desired_shape)
                    filtered_train.append(inp)
    ...
    #model.save_pretrained('./2d_tuned_model_tf')
    #print("Fine-tuned model saved to './2d_tuned_model_tf'")
